...
class PlayState extends MusicBeatState
{
  ...
  public static var noteBools: Array < Bool > = [false,
    false,
    false,
    false];
  ...
  public var dialogue: Array < String > = ['dad:blah blah blah',
    'bf:coolswag'];
  ...
  override public function create() {
    ...
    case 'lmao':
      dialogue = ["Hey you're pretty cute.",
  ...
  if (curStage == 'notch' && SONG.song.toLowerCase() == 'Bleeding')
    remove(notchStanding);
...
var introAssets: Map < String,
Array < String>> = new Map < String,
Array < String>>();
...
ready.screenCenter();
add(ready);
FlxTween.tween(ready, {
y: ready.y += 100, alpha: 0
...
ease: FlxEase.backInOut, startDelay: 0.5 + (0.2 * i)});
}
...
scoreTxTMovement = FlxTween.tween(scoreTxt.scale, {
x: 1, y: 1
}, 0.25, {
onComplete: function(twn: FlxTween) {
...
{
healthBar.createFilledBar(FlxColor.fromString('#' + dad.iconColor),
0xFF3e0707);
new FlxTimer().start(0.75,
function(swagTimer: FlxTimer) {
if (!paused) {
health -= 0.125;
healthLost += 0.125;
}
...
{
healthBar.createFilledBar(0xFFEAD127,
FlxColor.fromString('#' + boyfriend.iconColor));
health -= 0.25;
gappleActivated = true;
new FlxTimer().start(2,
function(gapple: FlxTimer) {
...
dad.playAnim("prepare",
true);
}

function alexUnequip() {
dad.playAnim("unequipPickaxe",
true);
}
...
blockWarning();
achievementBlock.animation.play('block',
true);
pressedSpace = false;
detectAttack = true;
...
FlxTween.angle(iconP1, -15, 0, Conductor.crochet / 1300 * gfSpeed, {
ease: FlxEase.quadOut
});
FlxTween.angle(iconP2, 15, 0, Conductor.crochet / 1300 * gfSpeed, {
ease: FlxEase.quadOut
});
}
...
'scale.x': 1, 'scale.y': 1
}, Conductor.crochet / 1250 * gfSpeed, {
ease: FlxEase.quadOut
});
